...
class TicketRepository {
    ...
    async findById(id) {
        const result = await database_1.pool.query('SELECT * FROM tickets WHERE id = $1', [id]);
    ...
    async update(id, ticket) {
      ...
      RETURNING *
    `;
        const result = await database_1.pool.query(query, [...values, id]);
        return result.rows[0];
    }
    async delete(id) {
        await database_1.pool.query('UPDATE tickets SET deleted = true WHERE id = $1', [id]);
    }
